...
export class AlboDOroComponent implements OnInit {
  ...
  private driverStates = signal<Record<number, SeasonDriverState>>(
    Object.fromEntries(
      SEASONS.map((season) => [season.id, { isLoading: true, podio: [], classifica: [] }])
    )
  );

  seasons = signal<SeasonDisplay[]>(
    SEASONS.map((season) => ({
      ...season,
      isLoading: true,
      driverPodio: [],
  ...
  private async loadChampionship(seasonId: number): Promise<void> {
    try {
      ...
      const sortedDrivers = [...drivers].sort((a, b) => Number(b.total_points) - Number(a.total_points));

      this.driverStates.update((states) => ({
        ...states,
        [seasonId]: {
          isLoading: false,
          podio: this.buildPodium(sortedDrivers),
          classifica: this.buildClassifica(sortedDrivers)
        }
      }));
    } catch (error) {
  ...
  private buildPodium(drivers: DriverData[]): PodiumEntry[] {
    return drivers.slice(0, PODIUM_SIZE).map((driver) => ({
